const { capitalize, reverse, calculator, caesarCipher, analyzeArray } = require('./index.js');
...
test('Adds two numbers', () => {
  expect(calculator.add(1.6, 2.1)).toBeCloseTo(3.7);
  expect(calculator.add(-1, 1)).toBe(0);
  expect(calculator.add(-10, -10)).toBe(-20);
});

test('Subtracts two numbers', () => {
  expect(calculator.subtract(3.3, 2.1)).toBeCloseTo(1.2);
...
test('Multiplies two numbers', () => {
  ...
  expect(calculator.multiply(-2, -0.5)).toBeCloseTo(1);
  expect(calculator.multiply(2, Infinity)).toBe(Infinity);
});

test('Divides two numbers', () => {
  expect(calculator.divide(36, 360)).toBeCloseTo(0.1);
...
test('Encrypts string with Caesar Algorithm', () => {
  expect(caesarCipher('CaeSar cipH3r', -12)).toBe('QosGof qwdV3f');
